...
module.exports = {
    ...
    markDatewise: async (req, res, next) => {
        try {
            const { selectedStudents, subjectCode, department,
                year,
                section, attdate } = req.body
            ...
            //Absent mark hone walo k liye
            for (let i = 0; i < filteredArr.length; i++) {
                const datewise = new Datewise({ student: filteredArr[i]._id, subject: sub._id, date: attdate, status: `Absent` })
                await datewise.save()
            }
            for (var a = 0; a < selectedStudents.length; a++) {
                const datewise = new Datewise({ student: selectedStudents[a], subject: sub._id, date: attdate, status: `Present` })
                await datewise.save()
            }
            res.status(200).json({ message: "done" })
        }
            ...
            console.log("error", err.message)
            return res.status(400).json({ message: `Error in marking datewise attendence${err.message}` })
        }
    },
