...
const Datewise = require('../models/datewise')
...
module.exports = {
    ...
    datewiseAttendence: async (req, res, next) => {
        try {
            const studentId = req.user._id
            const datewise = await Datewise.find({ student: studentId }).populate('subject')
            if (!datewise) {
                res.status(400).json({ message: "Attendence not found" })
            }
            res.status(200).json({
                result: datewise.map(att => {
                    let res = {};
                    res.subjectName = att.subject.subjectName
    ...
    getMarks: async (req, res, next) => {
        try {
            ...
            const InternalAssessment = getMarks.filter((obj) => {
                return obj.exam === "InternalAssessment"
            })
            const Practical_1 = getMarks.filter((obj) => {
                return obj.exam === "Practical_1"
            })
            const Practical_2 = getMarks.filter((obj) => {
                return obj.exam === "Practical_2"
            })
            const Activity = getMarks.filter((obj) => {
                return obj.exam === "Activity"
            })
            const ClassPresentation = getMarks.filter((obj) => {
                return obj.exam === "ClassPresentation"
            })
            const Viva = getMarks.filter((obj) => {
                return obj.exam === "Viva"
            })
            const OJT = getMarks.filter((obj) => {
                return obj.exam === "OJT"
            })
            res.status(200).json({
                result: {
